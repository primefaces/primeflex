...
function appendGradients(root, opts) {
    ...
    function appendColors(type) {
        const gradients = {
            [`${type}-from-to`]: `--gradient-from: var(--${type}-from-color); --gradient-to: var(--${type}-to-color, var(--${type}-from-color)00);`
        };
        const themes = getThemeColors(opts.themes, `${type}-`, p, `--gradient-${type}`);
        const whiteAndBlackRgba = getWhiteAndBlackRgbaColors(`${type}-`, `--gradient-${type}`);

        appendRules(root, gradients);
        appendRules(root, themes);
        appendRules(root, whiteAndBlackRgba);
    }
