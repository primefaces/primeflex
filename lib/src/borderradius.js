...
module.exports = (root, opts) => {
    const borderRadius = {
        'border-noround': '0',
        'border-round': `var(--${opts.prefix.cssVariable}border-radius)`,
        'border-circle': '50%'
    }
    ...
    edges.map(edge => borderRadiusEdges[edge] = {});

    for (let borderRadiusKey in borderRadius) {
        edges.map(edge => borderRadiusEdges[edge][borderRadiusKey + '-' + edge] = borderRadius[borderRadiusKey]);
    }

    for (let scale in scales) {
        edges.map(edge => borderRadiusEdges[edge]['border-round-' + edge + '-' + scale] = scales[scale]);
    }
