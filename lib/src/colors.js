...
module.exports = (root, opts) => {
    ...
    function generateGradientClasses(type, colorNames, colorShades, rgbaShades, opts) {
        ...
        for (let color of colorNames) {
            for (let cs of colorShades) {
                ...
                if (!colorValue) continue; // Skip to next iteration if colorValue is undefined
                ...
                for (let rgbaShade of rgbaShades) {
                    ...
                    if (isRgb) {
                        ...
                        rgbaValue = hexToRgba(colorValue, alpha); // Ensure hexToRgba function is defined and accessible
                    }
        ...
        // New: Default to-color to transparent version of from-color if to-color isn't specified
    ...
    for (let direction in gradientDirections) {
        gradients[direction] = `linear-gradient(${gradientDirections[direction]}, var(--gradient-from) var(--gradient-from-percentage, 0%), var(--gradient-to, var(--gradient-from),transparent) var(--gradient-to-percentage, 100%))`;
    }

    for (let direction in radialGradientDirections) {
        gradients[direction] = `radial-gradient(${radialGradientDirections[direction]}, var(--gradient-from) var(--gradient-from-percentage, 0%), var(--gradient-to, var(--gradient-from),transparent) var(--gradient-to-percentage, 100%))`;
    }
