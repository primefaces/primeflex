...
module.exports = (root, opts) => {
    ...
    for (let color of colorNames) {
        for (let cs of colorShades) {
            for (let rgbaShade of rgbaShades) {
                const opacity = rgbaShade;

                colored.text[`text-${color}-${cs}\\/${rgbaShade}`] = `color-mix(in srgb, var(--${p}${color}-${cs}) ${opacity}%, transparent)`;
                colored.background[`bg-${color}-${cs}\\/${rgbaShade}`] = `color-mix(in srgb, var(--${p}${color}-${cs}) ${opacity}%, transparent)`;
                colored.border[`border-${color}-${cs}\\/${rgbaShade}`] = `color-mix(in srgb, var(--${p}${color}-${cs}) ${opacity}%, transparent)`;
            }
        }
    }
    ...
    function generateGradientClasses(type, colorNames, colorShades, rgbaShades, opts) {
        ...
        for (let color of colorNames) {
            for (let cs of colorShades) {
                let colorValue = opts.themes['primeone-dark'].colors[color][cs];
                ...
                for (let rgbaShade of rgbaShades) {
                    const opacity = rgbaShade / 100;
                    const rgbaValue = `color-mix(in srgb, ${colorValue} ${opacity}%, transparent)`;
