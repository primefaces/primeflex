...
function getThemeColors(opts = {}, classNamePrefix = '', cssVariablePrefix = '', scopedCssVariable, hasRgba = true) {
    ...
    const defaultThemeName = names.find((name) => themes[name].default) || names[0];
    let classes = {};

    for (let themeName in themes) {
        if (themes.hasOwnProperty(themeName)) {
            const theme = themes[themeName] || {};
            const colorNames = Object.keys({ ...opts.colors, ...theme.colors });
            const isDefaultTheme = defaultThemeName === themeName;
            const computedName = theme.name || themeName;
            const themeClassNamePrefix = isDefaultTheme ? classNamePrefix : `${computedName}\\:${classNamePrefix}`;

            for (let color of colorNames) {
                color !== 'text' && Object.assign(classes, getColors(color, `${themeClassNamePrefix}${color}-`, `${cssVariablePrefix}${color}-`, scopedCssVariable, hasRgba));
            }
        }
    }
